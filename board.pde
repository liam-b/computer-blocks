    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++) {
        ...
        if (blocks[selectedLayer][x][y].mouseOver() && mousePressed && mouseButton == LEFT) {
          ...
          if (selectedType == EMPTY) blocks[selectedLayer][x][y].erase(blocks, selectedLayer);
          else blocks[selectedLayer][x][y].place(selectedType, selectedRotation, selectedLayer, blocks);
        }
